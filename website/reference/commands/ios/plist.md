...
源码：`objection/commands/ios/plist.py:12`

缺路径参数时返回错误 `CommandResult`（`objection/commands/ios/plist.py:21-33`）。相对路径解析见 `objection/commands/ios/plist.py:37-39`：
...
JSON 模式返回 `CommandResult(result={'path': plist, 'data': plist_data})`（`objection/commands/ios/plist.py:44-48`），其中 `path` 是解析后的绝对路径；非 JSON 模式 `click.secho(plist_data, bold=True)` 打印。
...
| 符号 | 位置 |
| --- | --- |
| `cat` | `objection/commands/ios/plist.py:12` |
