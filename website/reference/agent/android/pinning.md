...
| 平台 | Android（Java 层） |
| 导出的 RPC | `disable`（经 `agent/src/rpc/android.ts:84` 暴露为 `androidSslPinningDisable`） |
...
源码：`agent/src/android/pinning.ts:374`
...
源码：`agent/src/android/pinning.ts:22`
...
- **anti-Frida 规避**：`sslContextEmptyTrustManager` 改写 `tempFileNaming.prefix`，源自对 frida-java-bridge `class-factory.js` 的已知行为注释（源码 `agent/src/android/pinning.ts:44`）。
...
| 符号 | 位置 |
| --- | --- |
| `quiet` 模块标志 | `agent/src/android/pinning.ts:20` |
| `sslContextEmptyTrustManager` | `agent/src/android/pinning.ts:22` |
| 改写 `tempFileNaming.prefix` | `agent/src/android/pinning.ts:45` |
| `Java.registerClass` 注册空 TrustManager | `agent/src/android/pinning.ts:51` |
| `SSLContextInit.implementation` 替换 | `agent/src/android/pinning.ts:74` |
| `okHttp3CertificatePinnerCheck` | `agent/src/android/pinning.ts:88` |
| `okHttp3CertificatePinnerCheckOkHttp` | `agent/src/android/pinning.ts:138` |
| `appceleratorTitaniumPinningTrustManager` | `agent/src/android/pinning.ts:194` |
| `trustManagerImplVerifyChainCheck` | `agent/src/android/pinning.ts:240` |
| `trustManagerImplCheckTrustedRecursiveCheck` | `agent/src/android/pinning.ts:288` |
| `phoneGapSSLCertificateChecker` | `agent/src/android/pinning.ts:332` |
| `export const disable` | `agent/src/android/pinning.ts:374` |
